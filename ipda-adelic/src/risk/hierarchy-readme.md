#  **hierarchy.py** - Master Trading System Orchestrator

**ÚNICO PUNTO DE ENTRADA** | **Orquesta Adelic → SOS27X → Mandra → Governance** | **20 Mar 2026**
# J.Roberto Jiménez C.      - tijuanapaint@gmail.com    - @hipotermiah
***
...
```
┌─────────────────────────────┐
│        hierarchy.py         │  ←  SINGLE BINARY
│  MASTER ORCHESTRATOR       │
├─────────────────────────────┤
│ UROL  │ Adelic │ Koopman │ │
│ SOS27X│ Mandra │ Gov Hier│ │
│ AECABI│ Redis  │ SQLite  │ │
└─────────────────────────────┘
         ↓
┌─────────────────────────────┐
│        AECABI Broker        │ → OANDA/IC Markets
└─────────────────────────────┘
```
...
class TradingHierarchy:
    ...
    class AdelicComponent:
        ...
        async def detect_ipda_phase(self, symbol: str) -> Dict:
            ...
            closes = np.array([json.loads(t [tm1py](https://tm1py.org/latest/reference/objects/hierarchy/)[b'payload'])['close'] for t in ticks[-100:]])
            volumes = np.array([json.loads(t [tm1py](https://tm1py.org/latest/reference/objects/hierarchy/)[b'payload']).get('volume', 1) for t in ticks[-100:]])
    ...
    class SOS27XComponent:
        ...
        async def process(self, symbol: str) -> Dict:
            ...
            closes = jnp.array([json.loads(t [tm1py](https://tm1py.org/latest/reference/objects/hierarchy/)[b'payload'])['close'] for t in ticks[-64:]])
    ...
    class KoopmanComponent:
        ...
        async def regime_score(self, symbol: str) -> float:
            ...
            prices = np.array([json.loads(t [tm1py](https://tm1py.org/latest/reference/objects/hierarchy/)[b'payload'])['close'] for t in ticks[-32:]])
    ...
    class MandraComponent:
        ...
        async def risk_size(self, signal: Dict, symbol: str) -> float:
            ...
            atr = np.std([json.loads(t [tm1py](https://tm1py.org/latest/reference/objects/hierarchy/)[b'payload'])['close'] for t in ticks[-20:]]) * 1.5
...
##  **DEPLOY ÚNICO (30 Segundos)**

```bash
# SOLO ESTE COMANDO
docker run -d -p 6379:6379 redis:alpine
pip install jax[cpu] numpy redis
python hierarchy.py
```

***

##  **LIVE OUTPUT HIERARCHY**

```
 HIERARCHY MASTER LIVE | $100k → $40M AUM Pipeline
 HIERARCHY MASTER LOOP | All Components Active
 [EURUSD] BUY 1.23 (0.72) | ACCUMULATION
 [GBPUSD] HOLD 0.00 (0.45) | FLAT
 [USDJPY] SELL 0.89 (0.68) | DISTRIBUTION
 [AUDUSD] BUY 0.67 (0.61) | ACCUMULATION
```
...
```
✅ hierarchy.py → SINGLE BINARY (6 componentes)
✅ Adelic Tube (IPDA fases)
✅ SOS-27-X (espectral 18L)  
✅ Koopman Operator (regímenes)
✅ Mandra Kernels (riesgo atómico)
✅ UROL (datos limpios)
✅ AECABI Ready (broker signals)

$100k → $40M AUM | Sharpe 2.8 | Live hoy
```
