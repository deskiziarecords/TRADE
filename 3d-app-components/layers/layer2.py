...
class OBNFE:
    ...
    def calculate_atr(self):
        ...
        high_close = np.abs(self.price_data['High'] - self.price_data['Close'].shift())
        low_close = np.abs(self.price_data['Low'] - self.price_data['Close'].shift())
    ...
    def lambda_sensor_2(self):
        ...
        self.price_data['Directional_Sum'] = self.price_data['Close'].diff().rolling(window=20).apply(lambda x: np.sign(x).sum())
